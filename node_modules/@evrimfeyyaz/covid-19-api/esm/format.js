...
import { getDateKeys, getLocationInfoFromRow } from "./parse";
...
function getCanadaTotalData(globalParsedRecoveredDataCSV, canadaProvinceData) {
    ...
    var dateKeys = getDateKeys(globalParsedRecoveredDataCSV);
...
function getUSStateTotalsData(usCountyData) {
    ...
    var _loop_1 = function (state) {
        ...
        var location_1 = getFullLocationName("US", name_1);
        data.push({
            location: location_1,
            countryOrRegion: "US",
    ...
    for (var _i = 0, US_STATES_1 = US_STATES; _i < US_STATES_1.length; _i++) {
...
function formatParsedData(parsedConfirmedData, parsedDeathsData, parsedRecoveredData) {
    var dateKeys = getDateKeys(parsedConfirmedData);
    var data = [];
    for (var location_2 in parsedConfirmedData) {
        ...
        var _a = getLocationInfoFromRow(confirmedData), countryOrRegion = _a.countryOrRegion, provinceOrState = _a.provinceOrState, county = _a.county, latitude = _a.latitude, longitude = _a.longitude;
 ...
 */
export function formatGlobalParsedData(parsedGlobalConfirmedData, parsedGlobalDeathsData, parsedGlobalRecoveredData) {
 ...
 */
export function formatUSParsedData(parsedUSConfirmedData, parsedUsDeathsData) {
    var data = formatParsedData(parsedUSConfirmedData, parsedUsDeathsData);
    var usStateTotalsData = getUSStateTotalsData(data);
    return __spreadArrays(data, usStateTotalsData);
}
