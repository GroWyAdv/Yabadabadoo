...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `\`${this.name}\` command can be only used by the bot owner.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `\`${this.name}\` command requires you to have the \`${permissions[missing[0]]}\` permission.`;
				}
	...
	onBlock(message, reason, data) {
		switch(reason) {
			case 'guildOnly': {
				return SendErrorMsg(message, `command \`${this.name}\` must be used in a server channel.`);
			}
			case 'nsfw': {
				return SendErrorMsg(message, `command \`${this.name}\` can be used only in a NSFW channel.`);
			}
			case 'permission': {
				if(data.response) {
					return SendErrorMsg(message, data.response);
				}

				return SendErrorMsg(message, `you don't have permission to use \`${this.name}\` command.`);
			}
			case 'clientPermissions': {
				if(data.missing.length == 1) {
					return SendErrorMsg(message, `i need the \`${permissions[data.missing[0]]}\` permission for \`${this.name}\` command.`);
				}

				return SendErrorMsg(message, `i need the following permissions for \`${this.name}\` command: \`${data.missing.map(perm => permissions[perm]).join('`, `')}\`.`);
			}
			case 'throttling': {
				return SendErrorMsg(message, `you may not use \`${this.name}\` command again for another ${data.remaining.toFixed(1)} seconds.`);
			}
			default: {
				return null;
			}
