...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return message.channel.send(`**${message.author.username}**, command \`${this.name}\` can be used only by the bot owner.`);
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return message.channel.send(`**${message.author.username}**, command \`${this.name}\` requires you to have the \`${permissions[missing[0]]}\` permission.`);
				}

				return channel.send(`**${message.author.username}**, command \`${this.name}\` requires you to have following permissions: \`${missing.map(perm => permissions[perm]).join('`, `')}\`.`);
				return `\`${this.name}\` command requires you to have the following permissions: \`${missing.map(perm => permissions[perm]).join('`, `')}\`.`;
			}
		}
		return true;
	}
	...
	onBlock(message, reason, data) {
		...
		switch(reason) {
			case 'guildOnly': {
				return channel.send(`**${author.username}**, command \`${this.name}\` can be used only in discord servers.`);
			}
			case 'nsfw': {
				return channel.send(`**${author.username}**, command \`${this.name}\` can be used only on NSFW channels.`);
			}
			case 'permission': {
				if(data.response) {
					return;
				...
				return channel.send(`**${author.username}**, you're not allowed to use command \`${this.name}\`.`);
			}
			case 'clientPermissions': {
				if(data.missing.length == 1) {
					return channel.send(`**${author.username}**, i need \`${permissions[data.missing[0]]}\` permission to run the command \`${this.name}\`.`);	
				}
				
				return channel.send(`**${author.username}**, i need the following permissions to run the command \`${this.name}\`: \`${data.missing.map(perm => permissions[perm]).join('`, `')}\`.`);
			}
			case 'throttling': {
				return channel.send(`**${author.username}**, you can use \`${this.name}\` command in **${data.remaining.toFixed(1)} seconds**.`);
			}
			default: {
				return null;
			}
	 ...
	 */
	 isUsable(message = null) {
		...
		console.log(`isUsable(${messaage})`);
		
		const hasPermission = this.hasPermission(message);
		return this.isEnabledIn(message.guild) && hasPermission && typeof hasPermission !== 'object';
	}
